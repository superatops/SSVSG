...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.vrg_data_dir_attr = opt.vrg_data_dir_attr
        self.vrg_vocab = {v: k for k, v in json.load(open(opt.input_json))['ix_to_word'].items()}
        #print('*****self.vrg_vocab', type(self.vrg_vocab))
        #print('*****self.vrg_vocab', len(self.vrg_vocab))
        #print('*****self.vrg_vocab', self.vrg_vocab)

        # load the json file which contains additional information about the dataset
        self.logger.info('DataLoader loading json file: %s'% opt.input_json)
        self.info = json.load(open(self.opt.input_json))
        self.ix_to_word = self.info['ix_to_word']
        #print('*****self.ix_to_word', type(self.ix_to_word))
        #print('*****self.ix_to_word', self.ix_to_word)
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        #print('**********vrg_batch_data', type(vrg_batch_data))#vrg_batch_data <type 'dict'>
        #print('**********vrg_batch_data', vrg_batch_data)#vrg_batch_data {'rela_edges','attr_labels','obj_labels','rela_masks','rela_feats','verb_labels'}
        data['vrg_data'] = {k: v for k, v in vrg_batch_data.items() if k != 'verb_labels'}
        #print('**********data[vrg_data]', len(data['vrg_data']))#data[vrg_data] 5
        #print('**********data[vrg_data]', data['vrg_data'])#data[vrg_data] {'attr_labels','rela_edges','rela_masks','obj_labels','rela_feats'}
    ...
    def batch_vrg(self, vrg_batch, max_att_len):
        ...
        attr_batch = [_['attr'] for _ in vrg_batch]
        vrg_data = {}
        ...
        # attr labels, shape: (B, No)
        vrg_data['attr_labels'] = np.zeros([len(attr_batch), max_att_len, 2], dtype= 'int')
        for i in range(len(attr_batch)):
            vrg_data['attr_labels'][i, :attr_batch[i].shape[0]] = attr_batch[i]
    ...
    def get_graph_data(self, index):
        ...
        attr_use = np.load(self.vrg_data_dir_attr + image_id + '.npy')[()]
        ...
        #attr = attr_use['obj_info'][:, 1:]
        attr = attr_use['obj_attr'][:, 4:5]
        vrg_data = {'obj': obj, 'attr': attr, 'rela': rela, 'verb': np.unique(triplet_w[:, 2])}
        return vrg_data
...
class BlobFetcher():
    ...
    def _get_next_minibatch_inds(self):
        #print('**********_get_next_minibatch_inds')
